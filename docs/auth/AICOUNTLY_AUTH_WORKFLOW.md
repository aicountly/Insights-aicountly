# AICOUNTLY auth workflow (Insights)

How Insights signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `insights.aicountly.com` (or `insights.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/insights?returnUrl={origin}/auth/callback`.
...
| Insights host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `insights.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `insights.aicountly.com/api` |
| `insights.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `insights.gh.aicountly.com/api` |
...
`insights` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://insights.gh.aicountly.com/api/health
...
curl -i -X POST https://insights.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://insights.gh.aicountly.com/api/global/login   # expect 404
```
