 ...
 * against Books, Sales or Billing reads this API without a second set of rules:
 *
 *   single  {"data": {...}}
...
final class Http
{
     ...
     * @param array<string, mixed> $extra merged into meta (counts, filters echoed back)
     */
    ...
    /**
     * Replace the parsed body. CLI only — the seam the test suite posts through.
     *
     * @param array<string, mixed>|null $body
     */
    public static function setBodyForTesting(?array $body): void
    {
        if (PHP_SAPI !== 'cli') {
            return;
        }
    ...
    public static function boolParam(string $name, bool $default = false): bool
    {
        $raw = self::param($name);
        if ($raw === null || $raw === '') {
            return $default;
    ...
    /**
     * An array-valued parameter, from the JSON body or a repeated query key.
     *
     * @return list<mixed>
     */
    public static function arrayParam(string $name): array
    {
        ...
        if (isset($_GET[$name]) && is_array($_GET[$name])) {
            return array_values($_GET[$name]);
        }
        if (isset($_GET[$name]) && is_string($_GET[$name]) && $_GET[$name] !== '') {
            return array_values(array_filter(array_map('trim', explode(',', $_GET[$name])), static fn ($s) => $s !== ''));
        }
    ...
    /**
     * An object-valued parameter from the JSON body.
     *
     * @return array<string, mixed>
     */
    public static function objectParam(string $name): array
    {
        $body = self::body();
    ...
    /**
     * A correlation id for this request, stable for its whole life.
     *
     * Echoed on every error so a user reporting "it said something went wrong"
     * can be matched to the line in the log without guessing at timestamps.
     */
    public static function correlationId(): string
    {
        static $id = null;
        if ($id === null) {
            $inbound = self::header('X-Correlation-Id');
            $id = preg_match('/^[A-Za-z0-9._-]{8,64}$/', $inbound) === 1
                ? $inbound
