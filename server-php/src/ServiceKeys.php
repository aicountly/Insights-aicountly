 ...
 *
 * Configured as `SERVICE_KEYS=books:<key>,manage:<key>` in api/.env. The
 * comparison is constant-time: a plain === on a secret leaks its length and,
 * over enough requests, its content.
 *
 * Insights exposes very little to other products — it is a reader, not a
 * system of record — but the seam exists so a sibling can ask for a dashboard's
 * configuration without borrowing somebody's browser session.
 */
