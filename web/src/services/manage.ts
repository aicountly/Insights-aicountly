 ...
 * it, which is the only way a list of companies cannot go stale when somebody
 * is added to one elsewhere.
 *
 * The payloads are normalised by manageShapes.ts, which is shared verbatim with
 * Sales and Inventory: Manage's list envelope has grown several shapes over the
 * years and the row fields differ between `companies` and `companyinfo`.
 */
...
export async function fetchAllCompanies(signal?: AbortSignal): Promise<CompanyOption[]> {
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    const body = await api.unscoped<unknown>('v1/manage/companies', { filter: 'all', page, per_page: PER_PAGE }, signal)
